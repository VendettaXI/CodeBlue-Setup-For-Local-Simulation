...
export const PromptCard = React.memo(function PromptCard({
  ...
  return (
    <div 
      className="cb-card rounded-2xl p-4 sm:p-6 border group hover:shadow-lg hover:scale-[1.01] hover:border-blue-200 dark:hover:border-blue-700 transition-all duration-300 cb-reveal bg-[var(--cb-surface-muted)] text-gray-900 dark:text-gray-100" 
      style={{ animationDelay: `${index * 70}ms` }}
    >
      {/* Question - Responsive: 13px → 15px at sm+, Sans-Serif Bold, less prominent color */}
      <h3 className="text-[13px] sm:text-[15px] font-bold text-gray-500/90 dark:text-gray-400/90 mb-1.5 sm:mb-2 leading-tight cb-slightly-spaced">
        {question}
      </h3>

      {/* Answer - Responsive: 14px → 17px at sm+, Sans-Serif Light, slightly spaced */}
      {type === 'text' ? (
        <div className="flex items-center justify-between">
          <p
            className="text-[14px] sm:text-[17px] font-light leading-relaxed mb-4 sm:mb-5 text-justify cb-slightly-spaced"
          ...
          <button 
            ...
            className="ml-2 sm:ml-3 flex items-center justify-center transition-all hover:scale-110 flex-shrink-0"
            style={{ minWidth: '2.4rem' }}
          >
            {/* Pink heart outline icon - Responsive: 32px → 38px */}
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="sm:w-[38px] sm:h-[38px]">
        ...
        <button 
          onClick={onPlayVoice}
          className="w-full bg-gradient-to-r from-blue-600 to-blue-700 dark:from-indigo-500 dark:to-fuchsia-600 text-white px-4 sm:px-6 py-3 sm:py-4 rounded-xl font-semibold flex items-center justify-center gap-2 sm:gap-3 hover:from-blue-700 hover:to-blue-800 hover:scale-105 transition-all shadow-[0_4px_16px_rgba(37,99,235,0.25)] mb-4 sm:mb-5"
          aria-label={`Play voice note: ${answer}`}
        >
          {/* Play Icon */}
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm flex-shrink-0">
          ...
          <div className="text-left text-white">
            <div className="text-xs sm:text-sm font-bold truncate">{answer}</div>
            <div className="text-[10px] sm:text-xs opacity-90 font-medium">{duration}</div>
          </div>
        </button>
      )}
